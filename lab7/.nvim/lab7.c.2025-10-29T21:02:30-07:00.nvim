...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (!found) {
    output[upper_bound].doubled_value = input->doubled_value;
    output[upper_bound].line_numbers[0] = input->line_number;
    output[upper_bound].count++;
    *result_count += 1;
  }
  // TODO:
  // Group entries
