...
The client is using TCP and AF_INET to send data to the server
2. What data is the client sending to the server?
The client is sending "Hello", "Apple", "Car", "Green", and "Dog"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
It is given a struct that has a bool (decides whether to run acceptor or not)
and the head and count of a linked list, as well as a mutex for this linked list
2. How are received messages stored?
They are stored in a linked list
3. What does `main()` do with the received messages?
It counts how many messages were received and frees them. It also outputs a
message depending on if enough messages were received.
4. How are threads used in this sample code?
Threads are used to run the acceptor (server) and is also used to run the client
logic (to be filled)
Q5. Explain the use of non-blocking sockets in this lab
Sockets are made non blocking by calling the set_non_blocking function (which
calls fcntl and sets a non blocking flag). Both server and client sockets are
made non-blocking. By making sockets non blocking, we allow other sockets to get
through.
*/
#include <arpa/inet.h>
#include <errno.h>
