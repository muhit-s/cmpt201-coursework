...
int main() {
  ...
  FILE *file_stream = fopen("public_key.pem", "r");
  EVP_PKEY *pubkey = PEM_read_PUBKEY(file_stream, NULL, NULL, NULL);
...
#define MAX_FILE_SIZE 512
  ...
  FILE *message_stream = fopen(message_path, "r");
  int message_len =
      fread(message, sizeof(*message), MAX_FILE_SIZE, message_stream);

  FILE *signature_stream = fopen(sign_path, "r");
  int signature_len =
      fread(signature, sizeof(*signature), MAX_FILE_SIZE, signature_stream);

  EVP_MD_CTX *mdctx = EVP_MD_CTX_new();

  EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey);
  EVP_DigestVerifyUpdate(mdctx, message, message_len);
  int ret_final = EVP_DigestVerifyFinal(mdctx, signature, signature_len);
