...
port number). The address is 127.0.0.1 and the port is 8000
2. Is it UDP or TCP? How do you know?
It is TCP because we use connect on client.c (UDP does not use connect)
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? The client gets input from the read
function, i.e. terminal input
4. How does the client program end? How can you tell that in the code?
It ends when the input from the read function is of 1 or less length, i.e. the
user enters enter
*/
#include <arpa/inet.h>
#include <stdio.h>
...
void *handle_client(void *arg) {
  ...
  while (1) {
    char buf[BUF_SIZE];
    int status = read(client->cfd, buf, BUF_SIZE);
    if (status <= 0)
      break;
    buf[status] = '\0';
